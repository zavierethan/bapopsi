...
<section class="pt-16 bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mt-5">
        ...
        <input type="hidden" value="{{$subRayon->id}}" id="subRayonId" />
    </div>
</section>
...
$(document).ready(function() {
    let subRayonId = $('#subRayonId').val(); // get from hidden input or select on page load
    let cabangOlahraga = $('#caborId').val();

    loadSubRayonMedalTable(subRayonId, cabangOlahraga);

    $('#caborId').on('change', function() {
        cabangOlahraga = $(this).val();
        subRayonId = $('#subRayonId').val();
        currentPage = 1;
        loadSubRayonMedalTable(subRayonId, cabangOlahraga);
    });
...
function loadSubRayonMedalTable(subRayonId = '', cabangOlahragaId = '') {
    $.ajax({
        url: '/api/getAtletBySubRayon',
        method: 'GET',
        data: {
            sub_rayon_id: subRayonId,
