...
<body>
    ...
    <div class="idcard" id="idcard">
        ...
        <div>
            <img
                src="{{ $official->foto ? asset('storage/' . $official->foto) : 'https://ui-avatars.com/api/?name=' . urlencode($atlet->nama_lengkap) }}"
                class="idcard-photo"
                alt="Foto {{ $official->nama }}">
        </div>
        <div class="idcard-name">{{ $official->nama }}</div>
        <div class="idcard-sport">
            {{ $official->cabang_olahraga }} - {{ $official->nama_kecamatan ?? ($official->nama_sub_rayon ?? '-') }}
        </div>
        <div class="idcard-id">
            ID: {{ 'PORDA-' . str_pad($official->id, 5, '0', STR_PAD_LEFT) }}
        </div>
        <div class="idcard-qr">
            <div id="qrcode"></div>
        </div>
    ...
    <script>
        ...
        async function downloadAsPDF() {
            ...
            pdf.save('{{ \Illuminate\Support\Str::slug($official->nama) . ".pdf" }}');
        }
    </script>
